...
(async () => {
  await espera(700);
  const casos = [
    [2, {expansion:true, duelo:true, aprendizaje:true}, false],
    [3, {expansion:true, duelo:false, aprendizaje:false}, true],
    [4, {expansion:true, duelo:false, aprendizaje:true}, false],
    [6, {expansion:true, duelo:false, aprendizaje:false}, true],
  ...
  for (const [n, o, cortes] of casos){
    ...
    if (r.ok){ bien++;
      console.log(`✓ ${n} jugadores${o.expansion?" con expansión":" solo base"}${cortes?" y con caída de señal":""} → ganó ${r.ganador} tras ${r.turnos} acciones${r.errs.length?" · avisos: "+r.errs.slice(0,2).join("/"):""}`);
