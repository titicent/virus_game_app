...
function crearSala(nombre, ws) {
  const s = { codigo: codigo(), jugadores: [], opciones: {expansion:true, duelo:false, aprendizaje:false, halloween:false, segundosTurno:60, minutosJugador:0},
...
function vista(s, yo) {
  ...
  if (s.pendiente) {
    const p = s.pendiente;
    v.pendiente = { tipo: p.tipo, segundos: Math.max(0, Math.ceil((p.vence - Date.now())/1000)),
...
function empezar(s) {
  const E = {
    ...
    registro: ["Empieza la partida"], ganador: null, eventoN: 0, evento: null
  };
...
wss.on("connection", ws => {
  ...
  ws.on("message", datos => {
    ...
    if (m.t === "revancha" && s.iniciada && s.E.ganador !== null) {
      empezar(s); return difundir(s);
    }
    ...
    if (!s.iniciada || !s.E || s.E.ganador !== null || s.pendiente) return;
