...
const NOMBRES_BOT = ["Bacilo", "Espora", "Ameba", "Micrón", "Nucleína", "Priónix"];
...
function empezar(s) {
  const E = {
    ...
    metaInmune: !!s.opciones.metaInmune,
...
function cerrarTurno(s) {
  ...
  if (E.extra > 0) {                       /* horas extra / segunda opinión */
    E.extra--;
    if (E.extra > 0 && E.jugadores[E.turno].mano.length) {
      programarTurno(s);                   /* sigue el mismo jugador: hay que rearmar su reloj
                                              (y despertar al bot, que si no se queda dormido) */
      return;
    }
    E.extra = 0;
  }
...
function jugarSolo(s) {
  ...
  if (j.mano.length && !E.sinDescartar && !E.extra)
...
const AUTOGOL = ["infectar","extirpar","neutralizar","exp_extirpar","exp_quitar"];
...
function turnoBot(s) {
  ...
  if (mano.length && !E.sinDescartar && !E.extra) {
    E.descarte.push(mano.splice(0, 1)[0]);
    E.registro.push(E.jugadores[ji].nombre + " descartó 1 carta");
    E.evento = { n: ++E.eventoN, tipo:"descarte", ji };
  } else E.registro.push(E.jugadores[ji].nombre + " no tenía jugada y pasó");
  cerrarTurno(s); difundir(s);
}
/* Los bots también responden al traje de protección y a la reelección de objetivo. */
function botsResponden(s) {
  ...
  if (p.tipo === "traje") {
    p.esperando.filter(k => esBot(s, k)).forEach(k => {
      ...
      if (usa) {
        const t = s.E.jugadores[k].mano.findIndex(c => c.tr === "traje");
