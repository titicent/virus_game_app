...
/* El biónico cuenta como blindado: no se puede infectar y no admite medicinas. */
const blindado=o=>esBionico(o)||estadoOrgano(o)==="inmunizado";
const inmunes=j=>j.cuerpo.filter(blindado).length;
/* Progreso hacia la meta según el modo de la partida. */
const logrados=(E,j)=>E.metaInmune?inmunes(j):sanos(j);
...
function ganador(E){
  const i=E.jugadores.findIndex(j=>activo(j)&&!j.truco&&logrados(E,j)>=E.objetivo);
  return i<0?null:i;
}
...
function puntuar(E,ji,carta,jug){
  ...
  const lider=rivales.reduce((a,r)=>Math.max(a,r.truco?0:(E.metaInmune?inmunes(r):sanos(r))),0);
  ...
  const meta=E.metaInmune;
  const mide=x=>meta?inmunes(x):sanos(x);
  const misM=mide(yo);
  const gano=n=>!yo.truco&&misM+n>=E.objetivo;
  switch(jug.tipo){
    case "bajar":          return meta?46:(gano(1)?100:58+Math.min(20,mis*6));
    case "mutar":          return 40;
    case "curar":          return propio?(meta?58:(gano(1)?100:64+urg/2)):(yo.truco?70:10);
    case "exp_curar":      return propio?(gano(1)?100:66):12;
    case "superinmunizar": return propio?(meta?(gano(1)?100:90):80)+urg/2:(yo.truco?68:8);
    case "inmunizar":
    case "exp_inmunizar":  return propio?(meta?(gano(1)?100:88):72)+urg/2:(yo.truco?66:8);
...
function queHace(x){
  if(!x)return "";
  if(x.k==="organo"){
    if(x.c==="bionico")return "Órgano biónico: cuenta siempre como sano. Ningún virus lo infecta y no admite medicinas, pero te lo pueden robar o trasplantar.";
    if(x.c==="multi")return "Órgano comodín: vale como un órgano extra de cualquier color para completar tu cuerpo. A cambio, le afectan cartas de cualquier color.";
    if(x.c==="mutante")return "Órgano mutante: reemplaza a uno de tus órganos (el viejo se descarta) y solo lo tocan cartas multicolor y tratamientos.";
    return "Órgano "+etColor(x.c)+": colócalo en tu cuerpo. Reúne cuatro órganos distintos y sanos para ganar. No puedes tener dos del mismo color.";
  }
  if(x.k==="virus"){
    const e=x.t==="evo";
    return "Virus"+(e?" mutado":"")+" "+etColor(x.c)+": infecta un órgano rival de su color; un segundo virus lo destruye. Sobre un órgano vacunado, quita la vacuna."+
      (e?" Solo lo cura una medicina experimental o una cuarentena.":"");
  }
  if(x.k==="medicina"){
    const e=x.t==="exp";
    return "Medicina"+(e?" experimental":"")+" "+etColor(x.c)+": sobre un órgano tuyo libre lo "+(e?"blinda de un golpe (superinmuniza)":"vacuna")+
      "; sobre uno vacunado lo inmuniza para siempre; sobre uno infectado lo cura"+(e?", incluso de virus mutados":"")+".";
  }
  const D={
    trasplante:"Intercambia un órgano tuyo por uno de un rival. No vale si alguno queda con dos del mismo color o si está inmunizado.",
    ladron:"Roba un órgano de un rival y ponlo en tu cuerpo. No puedes robar órganos inmunizados ni repetir color.",
    contagio:"Pasa tus virus a órganos libres de los rivales, tantos como quepan, en una sola jugada.",
    guante:"Todos los demás descartan su mano y pierden su próximo turno robando de nuevo.",
    error:"Intercambias tu cuerpo entero con el de un rival, inmunizados incluidos. La única carta que mueve órganos blindados.",
    horas:"Juegas las otras dos cartas de tu mano en el mismo turno.",
    segunda:"Intercambias tu mano con la de un rival. Podrás jugar una de las cartas que recibas.",
    traje:"Guárdala: se juega cuando te atacan, para anular esa carta. No robas después de usarla.",
    cuarentena:"Retira del juego un virus (básico o mutado) de cualquier órgano hasta que termine la partida.",
    ladron_rojo:"Roba un corazón de un rival, aunque esté inmunizado.",
    ladron_verde:"Roba un estómago de un rival, aunque esté inmunizado.",
    ladron_azul:"Roba un cerebro de un rival, aunque esté inmunizado.",
    ladron_hueso:"Roba un hueso de un rival, aunque esté inmunizado.",
    alien:"Trasplante alienígena: intercambia dos órganos entre cualesquiera jugadores, sin importar si están inmunizados.",
    aparicion:"Cambia esta carta por la última del montón de descarte.",
    experimento:"Al jugarla eliges si actúa como virus o como medicina sobre cualquier órgano.",
    truco:"Maldice a un rival: no podrá ganar hasta que cure, vacune o inmunice un órgano de otro y le pase la maldición.",
    cambio:"Todos los jugadores pasan su cuerpo entero al de al lado, en el sentido que elijas."
  };
  return D[x.tr]||"";
}

const API={COLORES,ORG,TRAT,HALLOWEEN,activo,queHace,blindado,inmunes,logrados,crearMazo,barajar,nombreCarta,colorCarta,etColor,esHalloween,
