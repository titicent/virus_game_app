...
function conectar() {
  ...
  ws.onmessage = ev => {
    ...
    if (m.t === "vista") { V = m.v; sel = null; paso = null; recibido = Date.now(); restante = V.restante; pintar(); }
...
function pintar() {
  ...
  $mazo.textContent = V && V.iniciada ? "mazo " + V.mazo : "";
  if (!V) return portada();
  if (!V.iniciada) return sala();
  mesa();
  arrancarTic();
  V.pendiente ? pintarPendiente() : ($modal.innerHTML = "");
}
...
function sala() {
  ...
  ["oExp","oDuelo","oAp"].forEach(id => document.getElementById(id).onchange = enviar);
...
function sillaHTML(j, i) {
  ...
  return `<div class="silla ${V.turno===i?"activa":""} ${i===V.yo?"mia":""}">
...
function mesa() {
  ...
  h += `<div class="diario">${V.registro.slice().reverse().map(r => `<div>${esc(r)}</div>`).join("")}</div>`;
  h += `<div class="pie"></div>`;
  $app.innerHTML = h;
...
/* ── Atril de madera ────────────────────────────────────────── */
function atril() {
  if (V.ganador !== null) { $atril.innerHTML = ""; return; }
  const mio = miTurno();
  let aviso, acciones = "";
  ...
  if (mio && sel !== null) {
    ...
    if (!js.length) acciones += `<button class="acc tenue" disabled>Esta carta no tiene jugada legal ahora</button>`;
  ...
  $atril.innerHTML = `<div class="atril"><div class="tabla">
    ...
    <div class="cartas">${V.mano.map((c,i) => {
      ...
      return `<button class="carta ${sel===i?"elegida":""} ${mio&&!legales?"inerte":""}" data-c="${i}">
        <span class="banda" style="background:${R.colorCarta(c)}"></span>
        <span class="arte"><div>${A.dibujo(c, R.colorCarta(c))}</div></span>
        <span class="nom">${esc(nombreCorto(c))}<small>${esc(subtitulo(c))}</small></span></button>`;
    }).join("")}</div></div></div>`;
  ...
  const dibujar = () => {
    $modal.innerHTML = `<div class="telon"><div class="dialogo">
      ...
      <div class="cartas">${V.mano.map((c,i) => `<button class="carta ${marcadas.has(i)?"elegida":""}" data-d="${i}">
        <span class="banda" style="background:${R.colorCarta(c)}"></span>
        <span class="arte"><div>${A.dibujo(c, R.colorCarta(c))}</div></span>
        <span class="nom">${esc(nombreCorto(c))}</span></button>`).join("")}</div>
...
function pintarPendiente() {
  ...
  const arte = `<div style="width:74px;height:74px;margin:0 auto 10px">${A.dibujo(p.carta, R.colorCarta(p.carta))}</div>`;
  if (!p.mio) {
...
function cerrarModal(){ $modal.innerHTML = ""; delete $modal.dataset.traje; delete $modal.dataset.re; }
