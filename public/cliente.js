...
function sala() {
  ...
  const segs = [[0,"sin límite"],[15,"15 s"],[30,"30 s"],[45,"45 s"],[60,"1 min"],[90,"1 min 30"],[120,"2 min"]];
  ...
  $app.innerHTML = `
    ...
    <div class="mesa">${V.jugadores.map((j,i) => `
      <div class="silla ${i===V.yo?"mia":""}">
        <div class="avwrap">${A.avatar(i, 44)}${j.bot?'<span class="botsel">🤖</span>':""}</div>
        <div class="datos"><div class="linea"><span class="quien">${esc(j.nombre)}</span>
          ${j.bot?`<span class="mini">bot ${esc(j.bot)}</span>`:""}
          ${i===0?'<span class="mini">anfitrión</span>':""}
          ${i===V.yo?'<span class="mini">tú</span>':""}
          ${!j.bot&&!j.conectado?'<span class="mini off">sin señal</span>':""}</div>
          <div class="mini">${A.SILLAS[i%6].nombre}</div></div></div>`).join("")}
    </div>
    ${V.anfitrion ? `<div class="panel">
      <h2>Jugar contra la máquina</h2>
      <p>Añade rivales artificiales para jugar solo o para completar la mesa.</p>
      <div class="seg">
        <button data-bot="novato">+ Bot novato</button>
        <button data-bot="normal">+ Bot normal</button>
        <button data-bot="experto">+ Bot experto</button>
        ${V.jugadores.some(j=>j.bot)?`<button data-quitabot="1">− Quitar bot</button>`:""}
      </div>
      <p class="junto">El novato improvisa, el normal juega bien casi siempre y el experto
      siempre elige su mejor jugada. Ninguno ve tus cartas: deciden con lo que hay en la mesa.</p>
    </div>` : ""}
    <div class="panel">
...
/* Los órganos se reparten entre la mesa ($app) y tu propia zona ($atril),
   así que el clic se delega en el documento: cubre los dos. */
document.addEventListener("click", ev => {
  const b = ev.target.closest("[data-org]");
  if (!b || !V || !V.iniciada) return;
  const [ji, oi] = b.dataset.org.split(".").map(Number);
  if (!miTurno()) { infoOrgano(ji, oi); return; }
  if (sel === null) { infoOrgano(ji, oi); return; }
  tocarOrgano(ji, oi);
});
function infoOrgano(ji, oi) {
  const j = V.jugadores[ji], o = j.cuerpo[oi];
  if (!o) return;
  const est = R.estadoOrgano(o);
  const txt = R.esBionico(o) ? "Órgano biónico: siempre cuenta como sano. No lo infecta ningún virus y no admite medicinas."
    : est === "infectado" ? "Infectado con un virus " + (o.virus[0].t === "evo" ? "mutado: solo lo cura una medicina experimental o una cuarentena." : "básico: lo cura cualquier medicina de su color.")
    : est === "inmunizado" ? "Inmunizado: blindado para siempre. Ningún virus lo toca y no se puede robar ni trasplantar."
    : est === "vacunado" ? "Vacunado: para infectarlo, primero hay que quitarle la vacuna."
    : "Libre: cuenta como sano, pero se puede infectar.";
  $modal.dataset.abierto = "1";
  $modal.innerHTML = `<div class="telon"><div class="dialogo">
    <div class="fichagrande">${A.dibujoOrgano(o.carta.c)}</div>
    <h3>${esc(R.ORG[o.carta.c].label)}${ji===V.yo?" (tuyo)":" de "+esc(j.nombre)}</h3>
    <p>${txt}</p>
...
function tocarOrgano(ji, oi) {
  ...
  if (!j) return;
  const dañina = ["infectar","extirpar","neutralizar","exp_extirpar","exp_quitar"].includes(j.tipo);
  if (ji === V.yo && dañina) return confirmarAutogol(j);
  ...
  return `<div class="rival ${pos} ${activo?"activo":""} ${j.fuera?"ido":""}" data-silla="${i}">
    <div class="rcab">
      ...
      <div class="rinfo"><span class="rnom">${esc(j.nombre)}${j.bot?" 🤖":""}</span>
        <span class="rmini">${j.fuera?"fuera":j.cartas+" cartas"}${j.truco?" · 🎃":""}</span></div>
    </div>
...
function atril() {
  ...
  const manoHTML = `<div class="mano">${V.mano.map((c,i) => {
  ...
  const cuerpoHTML = `<div class="micuerpo">
      <span class="mimeta ${meta>=V.objetivo-1?"cerca":""}">${meta}/${V.objetivo}${V.metaInmune?" 🛡":""}</span>
      ${cuerpoMio.length?cuerpoMio.map((o,oi)=>fichaHTML(o,V.yo,oi,false)).join(""):'<span class="sinorg">baja órganos<br>para tu cuerpo</span>'}
    </div>`;

  $atril.innerHTML = `<div class="zonami">
    ${panel}
    <div class="filami">
      <div class="ladomano">${manoHTML}</div>
      <div class="ladocuerpo">${cuerpoHTML}</div>
    </div>
  </div>`;
...
/* El reglamento permite jugar un virus contra uno mismo, y a veces es obligatorio,
   pero conviene preguntar antes de que sea un resbalón del dedo. */
function confirmarAutogol(j) {
  $modal.dataset.abierto = "1";
  $modal.innerHTML = `<div class="telon"><div class="dialogo">
    <h3>¿Contra tu propio cuerpo?</h3>
    <p>Vas a <b>${esc(j.etiqueta.toLowerCase())}</b>. Es legal —el reglamento lo permite, y a
    veces es la única jugada posible— pero te perjudica a ti.</p>
    <div class="fila"><button class="btn warn" data-ok="1">Sí, hazlo</button>
    <button class="btn claro" data-no="1">Mejor no</button></div></div></div>`;
  $modal.querySelector("[data-ok]").onclick = () => {
    mandar({t:"jugar", idx:sel, jugada:j}); sel = null; paso = null; cerrarModal(); };
