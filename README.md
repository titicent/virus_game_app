...
## Cómo está armado
    ...
    pruebas-bots.js      partidas en solitario contra bots (npm run test:bots)
    pruebas-inactividad.js  expulsión tras 3 turnos sin jugar (npm run test:inactividad)
...
- **Jugar solo, contra la máquina.** En la sala, el anfitrión añade bots en tres niveles.
  El novato improvisa a menudo, el normal elige su mejor jugada tres de cada cuatro veces y
  el experto siempre juega lo mejor que ve. Ninguno mira las cartas ajenas: deciden con la
  misma información que tendría una persona, usando el mismo evaluador de jugadas que
  alimenta el modo aprendizaje. También responden al traje de protección y eligen otro
  objetivo cuando alguien se protege. La variable de entorno `PENSAR_BOT` (900 ms por
  defecto) marca cuánto se demoran en jugar, para que se note que "piensan".
- **Fuera por inactividad.** Si a alguien se le agota el tiempo de turno tres veces
  seguidas, queda fuera de la partida: sus cartas y órganos vuelven al descarte y la mesa
  sigue sin él. En los dos primeros avisa en el registro cuántos turnos le quedan, y
  cualquier acción propia pone el contador a cero. Solo aplica con límite de tiempo por
  turno activado; sin límite el reloj nunca salta. Los bots están exentos.
...
- **Tocar para entender.** Además del botón «?» de cada naipe, tocar cualquier órgano de la
  mesa cuando no tienes carta seleccionada explica en qué estado está y qué se puede hacer
  con él. Y si vas a jugar un virus contra tu propio cuerpo —el reglamento lo permite, y a
  veces es la única jugada legal— la app pide confirmación antes, para que no sea un
  resbalón del dedo.
