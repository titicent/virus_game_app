...
const ladronColor = c => `
  <path d="M30 5.5c-4.4 0-7.4 3.3-7.4 7 0 4.5 3.9 7.2 7.4 10.3 3.5-3.1 7.4-5.8 7.4-10.3 0-3.7-3-7-7.4-7Z" fill="${c}"/>
...
function caraCarta(carta, tono, grande){
  ...
  const lado = grande
    ? `<span class="lado izq">${r.titulo} / ${r.sub}</span><span class="lado der">${r.titulo} / ${r.sub}</span>`
    : "";
  return `<span class="cara" style="--tono:${tono}">
    <span class="cinta"><span class="ins">${dibujo(carta, "#FFFFFF")}</span>
      <span class="tit">${r.titulo}<b>${r.sub}</b></span></span>
    ${lado}
    <span class="ilustra"><span class="aura"></span><span class="obra">${dibujo(carta, tono)}</span></span>
    <span class="alpie">${r.titulo} / ${r.sub}</span></span>`;
}

const API = {dibujo, dibujoOrgano, avatar, colorSilla, SILLAS, claveCarta, caraCarta, rotulos};
