...
function nuevaPartida(n, expansion, objetivo, halloween){
  const E = {
    jugadores: Array.from({length:n},(_,i)=>({nombre:"J"+i,mano:[],cuerpo:[]})),
    mazo: R.barajar(R.crearMazo(expansion, halloween)), descarte:[], retiradas:[],
...
function jugarPartida(n, expansion, semilla, halloween){
  let rnd = semilla;
  const rand = () => (rnd = (rnd*1103515245+12345) % 2147483648) / 2147483648;
  const E = nuevaPartida(n, expansion, null, halloween);
...
let fallos=0, tipos={};
for (let s=1; s<=300; s++){
  const n = 2+(s%5), exp = s%3!==0, hal = s%2===0;
  try{
    const r = jugarPartida(n, exp, s*7919, hal);
