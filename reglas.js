...
function jugadasLegales(E, ji, idx){
  ...
  if(tr==="truco"){
    E.jugadores.forEach((otro,j)=>{ if(j!==ji&&!otro.truco)
      out.push({tipo:"truco",j,etiqueta:"Maldecir a "+otro.nombre+": no podrá ganar"}) });
    return out;
  }
  if(tr==="cambio"){
    if(E.jugadores.length>1){
      out.push({tipo:"cambio",sentido:1,etiqueta:"Todos pasan su cuerpo hacia la derecha"});
      if(E.jugadores.length>2)out.push({tipo:"cambio",sentido:-1,etiqueta:"Todos pasan su cuerpo hacia la izquierda"});
    }
    return out;
  }
...
function atacados(E,ji,jugada){
  ...
  if(t==="cambio")return E.jugadores.map((_,i)=>i).filter(i=>i!==ji);
...
function aplicar(E, ji, idx, jugada, protegidos){
  ...
  switch(jugada.tipo){
    ...
    case "cambio":{
      E.descarte.push(carta);
      const parte=E.jugadores.map((_,i)=>i).filter(i=>!prot.includes(i));
      if(parte.length>1){
...
function ganador(E){
  const i=E.jugadores.findIndex(j=>!j.truco&&sanos(j)>=E.objetivo);
  return i<0?null:i;
}
...
const API={COLORES,ORG,TRAT,HALLOWEEN,crearMazo,barajar,nombreCarta,colorCarta,etColor,esHalloween,
